...
- 実Cloudflare環境: DashboardテンプレートWorkerのみ。Terraform stackとD1 migrationは未適用
...
`cloudflare:remote:smoke`は公開dev Workerに一意なテストユーザーを作成し、端末2台・Bearer認証・Note・冪等性・cursor同期・PWAを検証します。検証用Pushと端末Bは終了時に削除／失効します。
