...
- 実Cloudflare dev環境: Terraform remote stateでWorker、D1、非公開R2、Durable Object、Turnstile、Cron、Static Assets、Accessを管理。dev D1はmigration 0001〜0010、Phase 7 E2EE Worker/PWAを適用済み。Service Auth経由の公開HTTP・暗号化Note/File・private R2 remote smokeに合格
...
`cloudflare:remote:smoke`はAccess許可済みの実行元から、公開dev Workerに一意なテストユーザーを作成し、端末2台・Bearer認証・P-256 device envelope・暗号化Note/File・冪等性・cursor同期・private R2 byte一致と復号・PWAを検証します。検証用Pushと端末Bは終了時に削除／失効します。
