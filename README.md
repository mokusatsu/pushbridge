...
`cloudflare:remote:e2e`は公開PWAを実Chromiumで検証します。Accessで許可した固定IPからはService Tokenなしで、IP外からはService Tokenで短期Access cookieをbrowser contextへ保持し、Service Worker script、WebSocket、offline reloadも同じAccess sessionで検証できます。2026-07-24に固定IPv4からtokenヘッダーなしで暗号化Note/File、IndexedDB、server削除後Blob保持、Service Worker、offline reloadまで合格しました。Playwright trace／screenshot／videoは無効で、Access tokenを成果物へ保存しません。
