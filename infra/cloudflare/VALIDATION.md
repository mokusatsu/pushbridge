...
| PWA契約検査、TypeScript、Vitest、production build | 合格（Vitest 38件。E2EE固定vector、AAD改変、wrong key、nonce再利用防止を含む） |
...
| Worker TypeScript、bundle、公式Workers Vitest pool | 合格（26件。Web Push、retention fault injection、Passkey/session、device link、E2EEを含む） |
| RelayMock実HTTP smoke | Note、Link、File、subscriptionを含め合格 |
| Terraform fmt / validate | Cloudflare Provider v5.22.0で合格 |
| Cloudflare local smoke | D1 migration 0001〜0010、private R2、2端末、Bearer、暗号化Note/File、P-256 envelope、端末B復号、PWA／SPA／Service Worker ACK codeに合格。Passkey Playwrightも合格 |
...
dev CapabilitiesはE2EEとWeb Push deliveryを`true`、Passkey cookie sessionとrealtimeを`false`、uploadを`server-ticket`／`direct_upload=false`として実装・実測範囲に一致させている。
