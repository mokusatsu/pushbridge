...
| RP-003 Cloudflare統合test | PoC完了。公式Workers Vitest poolでD1 migration、R2、DO、Web Push、retention、Passkey、E2EE、account deletion、R2 SigV4と主要routeを40件検証 |
...
| RP-601〜605 Worker File／R2 | Phase 2〜7 PoC実装済み。private R2 server-ticket、File Push、25 MiB境界、削除／410、client-side PBFE暗号化を統合test。短寿命SigV4 PUT/GET adapterはdormant deploy済みで、専用資格情報と実R2 direct E2Eが未完了 |
...
Cloudflare devではTerraform、D1 migration 0001〜0012、TypeScript Worker bundle、PWA、Web Push／retention／E2EE／realtime／account deletion binding、Accessを適用済み。固定IPv4とService Authのremote smoke、schema version 12、公開ブラウザーService Worker／IndexedDB／offline、post-apply Plan差分なしを確認した。実Edgeの短命profileでは本物のPushManager subscriptionを作成し、PWA終了中の暗号化File自動保存、IndexedDB commit後cached ACK、サーバー削除後offline Blob保持まで合格した。Cloudflare Scheduled datasetで実Cronのsuccessも確認済み。残る外部入力は専用R2 S3資格情報、通知配送先／GitHub Actions secrets、Custom Domain/Passkey RP IDである。
...
状態: server-ticket PoCに加えて短寿命SigV4 PUT/GET、上書き防止、complete時実bytes hash照合、専用tfvars renderer、rotation境界を実装しdormant deploy済み。専用最小権限credentialの発行と実R2 direct E2Eは未完了。
...
状態: `init`／PUT ticket／`complete`、固定TTL、Worker生成prefix、size/hash/存在検査に加え、PWA向けSigV4 PUT、署名済み`If-None-Match: *`、30秒GET交換を実装済み。専用資格情報がないdevではserver-ticket fallbackであり、direct upload実環境完成とは扱わない。
...
状態: 実装・dev実測済み。PWAとChromium拡張が25 MiB以内のFileをPBFE containerへAES-256-GCM暗号化し、opaque filename/content type、ciphertext hash、progress/cancel/retryを扱う。Wrangler local、remote smoke、公開Chromium、closed-PWA実Web Pushでpeer復号とR2平文非漏洩を確認。
...
状態: dev実測済み。Cronとupload reservationから同じ冪等cleanupを実行し、stale pending、TTL、容量逼迫、R2 delete retry、alias/tombstone完全回収を実装。migration 0012でaccount deletion jobを追加し、即時認証停止、WebSocket切断、R2 batch削除、D1物理削除、指数backoff再試行をWorker統合テストで確認。
...
状態: PoC実装済み。account/device/IP mutation rate limit、UTF-8 payload上限、8 GiB storage budget、予約bytes、429／507、日次KiB-second履歴をWorker統合testとremote capacity baselineで確認。
...
状態: 部分完了。storage pressureのnormal／notice／constrained／emergency、95% cleanup開始、85% target、File優先削除、507 write protectionを実装。汎用admin overrideと独立`/service-status`は未実装。
...
状態: 部分完了。Cloudflare GraphQLのWorker/D1/R2実測、Cron Scheduled dataset、固定上限synthetic monitor、GitHub scheduled workflow、optional incident／Service Token期限notificationを実装。通知メールとrepository secretsが未設定で、配送実測は未完了。
...
状態: PoC完了。remote state消失からのimport復旧、D1 export→隔離D1 restore、migration 0012、暗号文・件数照合、一時成果物回収を自動化・記録。production R2全量inventory／restoreはdev PoC対象外。
...
状態: dev実測完了。即時session/device/token/subscription失効、DO切断、R2 100件cursor batch、D1物理削除、指数backoff、20回後manual intervention、PWA IndexedDB秘密消去を実装しremoteで両端末token 401と完了jobを確認。
...
状態: local／dev実測完了。二端末登録、Aから暗号化Note、DO tickle、Bのcursor同期、offline中の欠落回復、Idempotency-Key再送、端末失効をWorker smokeと実Chromiumで確認。
...
状態: server-ticket方式でlocal／dev実測完了。暗号化File、private R2、Service WorkerのIndexedDB自動保存、cached ACK、サーバー削除後offline Blob、未保存端末missedを確認。presigned direct方式の実R2 E2EはRP-601の資格情報待ち。
...
状態: PoC範囲を自動化済み。cross-user IDOR、signed cursor改変、session revoke、Origin/RP ID/replay、XSS危険URL、R2 host制限、任意key、size/hash、E2EE AAD/wrong key/nonce、Access denyを検証。第三者security reviewは未実施。

### RP-904 [P2/M] Load/capacity test

状態: 基準値取得済み。dev固定IPv4からconcurrency 10、同一Idempotency-Key 50並行再送、health 100、cursor 100、error 0、一意Push 1件を確認。5,000 Push/day長時間soak、reconnect storm、direct R2負荷は未実施。
...
状態: 進行中。runbook、retention、state/D1 recovery、privacy draft、account deletion、metrics基準値、適用後drift確認は完了。Custom Domain/RP ID、direct R2 credential/E2E、通知配送、privacy/support公開先、第三者security reviewがgateとして残る。
