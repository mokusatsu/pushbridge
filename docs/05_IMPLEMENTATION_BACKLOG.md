...
| RP-701〜702 Chromium拡張 | local PoC完了。Manifest V3最小権限、device-link、P-256端末鍵、E2EE Note/Link、current tab、selection/context menu、端末選択、shortcut、再現可能zipを実装。Wrangler＋実Chromiumでpeer復号まで検証 |
...
状態: 部分完了。context menu／shortcutの送信結果notificationは実装済み。拡張service workerのWebSocket reconnect、受信cursor同期、PWAとの通知担当調停は未実装。
