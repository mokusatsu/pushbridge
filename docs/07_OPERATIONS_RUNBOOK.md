...
公開APIの定時synthetic監視は、Cloudflare Access Service Tokenをrepository
secretとして使う`.github/workflows/cloudflare-monitor.yml`が毎時17分に
`healthz`、bootstrap bindings、capabilitiesを検査する。5秒を超える応答、
HTTP/JSON異常、D1／R2／Durable Object binding不足、E2EE／realtime／Web Push
capability低下でworkflowを失敗させる。ローカル再現:

```bash
npm run cloudflare:remote:monitor
```

Cloudflare全体のincidentとAccess Service Token期限通知は、operatorの配送先が
確定している場合だけ未追跡tfvarsへ`notification_email`を設定する。Terraformは
`cloudflare_notification_policy`を作る。`null`では通知資源を作らず、勝手な
メールアドレスを設定しない。

R2直接転送はTerraform backendとは別のObject Read & Write資格情報を使う。
`env.txt`へ`R2_S3_ACCESS_KEY_ID`と`R2_S3_SECRET_ACCESS_KEY`を保存後、値を
出力しないrendererでignored tfvarsを作る:

```bash
npm run cloudflare:r2-direct:render
```

PUTは2分、download ticket交換後のR2 GETは30秒。PUTでは署名済み
`If-None-Match: *`により上書きを拒否し、ブラウザーはSHA-256を認証済み
complete APIへ渡す。Workerは暗号文本体と初期化値を照合する。presigned URLは
bearer tokenで失効までは再利用できるため、標準出力、trace、artifactへ残さない。
Chromium拡張は最小host permission維持のためserver-ticketを使う。

公開devのcontrol planeを、既存データに触れず固定上限で確認する:

```bash
npm run cloudflare:remote:load-evidence
```

この検証は短命の合成accountを作り、既定でconcurrency 10、同一
`Idempotency-Key` 50並行再送、health 100回、認証済みcursor read 100回を
実行する。作成されるPushは一意に1件でなければ失敗し、終了時にaccountを
完全削除してtokenの401失効まで確認する。本文、token、request/response bodyは
出力しない。件数環境変数には安全上限があり、dev以外へ向けて実行しない。
