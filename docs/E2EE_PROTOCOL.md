...
As of 2026-07-23, Worker, PWA, Service Worker, OpenAPI, fixed vectors, and the two-browser Wrangler E2E pass locally with `REQUIRE_E2EE=true`. Dev migrations `0009` and `0010` and the matching Worker plan are applied: schema version 10, post-apply Terraform plan has no changes, and the Service Auth remote smoke verifies P-256 device envelopes, encrypted Note/File metadata, private R2 byte identity, and peer decryption. A full remote Service Worker/offline browser run must originate from an Access-allowlisted IP because Service Worker script requests cannot carry the service-token headers used by headless automation.
