...
As of 2026-07-24, Worker, PWA, Service Worker, OpenAPI, fixed vectors, and the two-browser Wrangler E2E pass locally with `REQUIRE_E2EE=true`. Dev migrations `0009` through `0012` and the matching Phase 9 Worker/PWA are applied: schema version 12, post-apply Terraform plan has no changes, and remote smoke verifies P-256 device envelopes, encrypted Note/File metadata, private R2 byte identity, peer decryption, a Durable Object WebSocket tickle, and complete account erasure. Remote Chromium also passes encrypted Note/File, IndexedDB persistence, server deletion with local Blob retention, Service Worker activation, and offline reload. A real Edge PushManager subscription additionally passes closed-PWA delivery, Service Worker decryption, IndexedDB commit before cached ACK, and offline Blob retention.
