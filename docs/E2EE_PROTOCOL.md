...
As of 2026-07-23, Worker, PWA, Service Worker, OpenAPI, fixed vectors, and the two-browser Wrangler E2E pass locally with `REQUIRE_E2EE=true`. Dev migrations `0009` through `0011` and the matching Phase 8 Worker plan are applied: schema version 11, post-apply Terraform plan has no changes, and the Service Auth remote smoke verifies P-256 device envelopes, encrypted Note/File metadata, private R2 byte identity, peer decryption, and a Durable Object WebSocket tickle. Remote Chromium also passes encrypted Note/File, IndexedDB persistence, server deletion with local Blob retention, Service Worker activation, and offline reload by retaining the short-lived Access cookie issued for the browser context.
